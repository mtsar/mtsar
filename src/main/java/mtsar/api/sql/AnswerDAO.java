...
public interface AnswerDAO {
    ...
    @SqlQuery("insert into answers (process, datetime, tags, worker_id, task_id, answers) values (:process, coalesce(:dateTime, localtimestamp), cast(:tagsTextArray as text[]), :workerId, :taskId, cast(:answersTextArray as text[])) returning id")
    int insert(@BindBean Answer a);

    @SqlBatch("insert into answers (id, process, datetime, tags, worker_id, task_id, answers) values (coalesce(:id, nextval('answers_id_seq')), :process, coalesce(:dateTime, localtimestamp), cast(:tagsTextArray as text[]), :workerId, :taskId, cast(:answersTextArray as text[]))")
    @BatchChunkSize(1000)
